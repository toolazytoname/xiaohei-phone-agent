...
| OpenCode 复杂任务 / OpenCode complex tasks | `FOUNDATION`; `OC-002–006` done | 手机上的 OpenCode TUI/Web、独立模型 profile、三类型私有任务协议、任务私有路径隔离、有界 runner、脱敏进度卡及停止/清理协调器 | `OC-007`; restricted tool policy before any real execution |
...
1. `OC-007` → `OC-008` — 接入受控 OpenCode 能力，不把原始 shell 暴露给模型。
