...
[中文总纲](docs/sovereign-mobile-agent-master-plan.zh-CN.md) · [English master plan](docs/sovereign-mobile-agent-master-plan.md) · [自由语音聊天计划](docs/free-voice-chat-delivery-plan.zh-CN.md) · [Free-voice plan](docs/free-voice-chat-delivery-plan.md) · [中文任务账本](docs/execution-backlog.zh-CN.md) · [English backlog](docs/execution-backlog.md) · [证据矩阵](docs/delivery-evidence-matrix.zh-CN.md)
...
| 开放对话 / Open conversation | `VERIFY`; `FVC-000/010/020/030/110A` complete; `FVC-040A/050A/060A/070A` code gates complete; physical gates pending | [脱敏基线](docs/acceptance-fvc-000.zh-CN.md)、[配置隔离](docs/acceptance-fvc-010.zh-CN.md)、[L1 真实闭环](docs/acceptance-fvc-020.zh-CN.md)、[ASR profile](docs/acceptance-fvc-030.zh-CN.md)、[单轮](docs/acceptance-fvc-040.zh-CN.md)、[多轮](docs/acceptance-fvc-050.zh-CN.md)、[DSP 入口](docs/acceptance-fvc-060.zh-CN.md)、[中断资源](docs/acceptance-fvc-070.zh-CN.md)及 [AOSP mock](docs/acceptance-fvc-110.zh-CN.md)证明隔离/状态机/半双工/DSP 入口/释放接线与无模型两轮回归；私有含模型 `alpha.5` 已安装，CPU KWS OFF、DSP ACTIVE | OnePlus 仍需 `FVC-040B/050B/060B/070B` 的真人声学、电话、route 与资源门禁 |
...
1. `FVC-040B/050B/060B/070B` — OnePlus 上完成单轮、两轮、DSP、电话/route 的真人和资源门禁；自动结果不能替代。
2. `FVC-080/090/120 → FVC-130` — 真人开放中文 ASR A/B、路由矩阵、产品验收、双语发布与最终合并；没有合法实测的自定义 DSP 资产前不得宣称息屏“小黑小黑”可用。
...
| `CHAT-012` | 真人确认中文 TTS 可懂、打断自然且无录音残留 | 自动测试只能验证生命周期，不能判断体验 |
...
- `REL-004`：已发布仅模拟器、仅 debug 的 100 条混合路由压力 harness；它要求命令精确一次、聊天/复杂任务只打开草稿、歧义不执行，且拒绝 crash/ANR/录音残留。当前桌面执行器的脱离 shell 使 AOSP 尝试无效，尚无可接受的实跑样本，保持 `VERIFY`。
...
- `ROOT-010`：干净 Android 14 ARM64 AOSP 独立设备已完成 source-only 的 code 4 安装、code 5 升级、普通降级拒绝、显式维护回退及事务卸载；无主包、Assistant role 或 active recorder 残留。该项没有执行 root，也不代表真实 root adapter 已存在。
...
- `TOOL-003`：目录上限内超时写入调用摘要，成功授权只产生一个私有、一次性执行许可；协调器最多提交一个注入式适配器，统一返回私有有界结果。25 组 Java 精确通过 5 成功、5 到期、5 取消、5 类型化失败与 5 拒绝/重放，9 个运行 worker 明确认收中断；另有本机确认流测试，只能为 `Pictures/XiaoheiTest/` 项目数生成一次 query capability。应用页面默认取消、只读且不暴露路径/内容/复制/删除/root/network；真实设备、可变适配器回滚、外部 peer 与资源关闭仍无证据，准确保持 `VERIFY`。
- `TOOL-002`：一次成功确认的私有 receipt 只能换取一个纯内存、一次性、默认 `SecureRandom` 128-bit、1–30 秒 capability；它绑定 confirmation/task/request/plan/call/工具/版本/风险/受众/参数/幂等/时间/超时/隐私摘要。50 组 Java 与 7 个公开 fixture 文件拒绝 10 个非 loopback/跨 UID、确认复用/错范围、7 种调用漂移、3 种目录漂移、5 种非法/未来/陈旧/私有元数据调用、精确到期/时钟回退、重放/撤销及跨网关；活动/重放状态有界，模型/动作/执行调用均为 0。授权核心仍没有监听器或远端 API；`TOOL-003` 的本机只读页面是唯一消费私有执行许可的可见调用方。
...
- `POLICY-002`：纯内存一次性确认门只接受前台、可交互、已解锁状态下的本机用户手势，并绑定 task/request/plan、加盐目标/内容摘要及 1–60 秒单调时窗；50 条矩阵覆盖 10 次精确单次放行、15 种范围变化、过期/时钟、设备、10 条助手伪造和重放/取消，全部零模型/动作调用。4 个公开夹具拒绝助手来源、非法时窗与原始内容；现仅由 `TOOL-003` 的本地只读测试查询页消费，未接模型、远端或泛化执行器。
...
- `ROUTE-004`：只有有类型的用户复杂任务才能生成不可变 schema-v1 请求，字段固定为 high-risk、requires-confirmation、pending、dry-run；39 条矩阵中 10 条助手确认伪造、10 条聊天/短命令、5 条歧义与 4 条非法边界均零请求或回到追问，固定零模型/动作调用。主页复杂路由只预填 Phone Agent；用户点击请求规划时先创建该 pending request，规划结果必须绑定同一 request，之后仍须用户确认且未接通工具执行。AOSP 渠道关闭路径本地失败、零规划请求。
- `ROUTE-003`：纯本地澄清策略精确通过 50 条合成非私人文本：10 条多目标、10 条动作意图、10 条范围缺失全部变成聊天加未知命令的追问，20 条明确输入正常放行；固定零猜测动作、零模型/动作调用。主页现显示澄清而不跳转或执行；AOSP 验证歧义文本保持“需要澄清；没有执行”。
- `ROUTE-002`：纯三路分类器精确通过 100 条合成非私人文本（40 确定性命令、35 聊天、25 复杂任务）；“回复消息是什么意思”不再被旧关键词误当草稿，“打开相册和相机”保持惰性，高风险复杂请求只分类不授权。主页已接入：聊天只预填 Conversation，复杂任务只预填 Phone Agent，二者均零模型/动作调用；AOSP 用户路径已验证。
...
- `UX-005` 应用级接线：停止中心现跨页面保存显式所有者并支持注销；Conversation 已登记在途请求与 TTS，活跃 Phone Agent 任务也在开始后登记、终态或服务销毁时释放，主页/通知触发时会调用既有停止路径。工具已登记；OpenCode、root 与真机资源归零仍为 VERIFY。
- `UX-005` 工具接线：已受权工具执行会登记同一取消信号；全局停止会请求 `GLOBAL_STOP` 并由有界协调器中断 worker。Phone Agent、OpenCode、root 与真机资源归零仍为 VERIFY。
- `VOICE-005`：新增来电/闹钟/媒体/Activity 的统一信号中断策略，固定停止输入/输出、释放所有权且不自动恢复；主页暂停已通过它停止 ASR。尚未将真实来源和 TTS 全部接线，保持 VERIFY。
- `TOOL-012`：工具结果证据门现已接到受限无障碍的每次点击/返回/滚动；适配器确认后必须出现更新且预期包名匹配的观察，旧快照、抢切包名、适配器失败和重复验证都拒绝。Android 14 AOSP 已完成 `文件 → Documents` 的一条真实语义路径并得到 `VERIFIED`；真机/App 版本矩阵和通用适配器结果仍保持 VERIFY。
...
- `TOOL-003` 接线：封闭 Android 工具注册表只能解析受审的 MediaStore 测试和日历测试适配器；页面只接入默认取消的本地只读测试目录查询，不能让模型绕过网关。真实权限、可变适配器、外部调用方与设备证据保持 VERIFY。
- `TOOL-003` bridge：现有一次性网关授权可进入有界协调器并解析封闭 Android 注册表；本机前台确认可调用只读 query，实际设备执行与其余工具仍为 VERIFY。
