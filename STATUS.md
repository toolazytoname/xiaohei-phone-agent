...
| 受控 root / Controlled root | `FOUNDATION`; `ROOT-001–004,008–009` done | root 治理、固定只读 action 目录、内存 signer/一次性 broker、脱敏诊断投影、服务停止 dry-run 精确核验、脱敏审计/撤销与破坏性请求拒绝库已完成；真实 root adapter 尚未实现 | `ROOT-005`; backup before any mutable root action |
