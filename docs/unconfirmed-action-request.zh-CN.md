...
本基础目前有意不接入 `MainActivity` 或 `ConversationActivity`。`PLAN-001` 已定义有边界 dry-run 计划，`POLICY-002` 已用独立一次性新鲜确认票据实现确认，而不修改模型可见请求文字。可见执行前仍需 `PLAN-002`、`UX-004` 与工具/能力门禁；请求自身继续停在 `pending`。
