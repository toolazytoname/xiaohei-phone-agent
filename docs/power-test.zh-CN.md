...
3. 先按电源键熄屏，再在十分钟内物理拔掉 USB/电源。部分设备配置了“充电时保持唤醒”，只拔线可能无法在预检窗口内自动熄屏。只有 Android 同时报告 AC、USB、无线供电均为 false、无活跃通话且屏幕不处于交互唤醒后才开始计时。原始 TSV 同时记录请求时间、实际开始采样时间和前置等待时长，不能把主机发起脚本的时间误当成静置开始。
