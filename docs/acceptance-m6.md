...
The candidate is not a public Beta yet. Required evidence still missing: 8–24 hour unplugged idle/power, an independent malware-engine review beyond static/OSV checks, encrypted offline key recovery before public release, model redistribution approval, a real user-config backup/restore drill, and public-release provenance. The 25-surface launch matrix does not replace the separate 10–15-app Phone Agent task matrix.
