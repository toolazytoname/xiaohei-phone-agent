...
| VOICE-010 | DONE | VOICE-004 | 代际化句子队列已接入系统 TTS 适配器：立即提供首句，仅在当前完成后推进；替换、停止、中断或销毁会使已排队/迟到完成事件失效。OnePlus 真机记录了 24 kHz 跨句有序输出及中途停止后丢弃余下三句；见 `acceptance-voice-010.zh-CN.md` | 队列取消不继续播报过期文本 |
| VOICE-011 | HUMAN | VOICE-010 | 对话页已有独立可见的“停止播报”控制；仅在选中系统 TTS 时播报已接受/重说的回复，停止、清空和生命周期路径均会打断且不会自动恢复。OnePlus 引擎可见输出在控制事件后 227 ms 停止且剩余队列被取消；真人确认 ≤300 ms 可听打断及回声循环资格仍为必需门禁；见 `acceptance-voice-011.zh-CN.md` | 打断不续播旧文本；语音 barge-in 只作后续实验 |
...
| CHAT-005 | VERIFY | CHAT-004,VOICE-002 | 独立开放 ASR、单轮/多轮按键语音和 DSP/中断代码门禁已完成；见[自由语音聊天可执行计划](free-voice-chat-delivery-plan.zh-CN.md) | 仍须真人提问 → 文本 → 模型 → 离线 TTS、全局停止、输入/输出不重叠和不自动开麦的 AOSP/OnePlus/HUMAN 证据 |
...
| ROUTE-002 | DONE | CHAT-004 | 三路分类：聊天 / 确定性命令 / 复杂任务 | 精确 100 条：40 命令保留既有动作路径、35 聊天只预填 Conversation、25 复杂任务只预填可见 Phone Agent；所有交接零模型/动作调用 |
| ROUTE-003 | DONE | ROUTE-002 | 低置信度只追问，不猜测执行 | 纯本地精确通过 50 条矩阵：10 目标、10 意图、10 范围追问及 20 明确对照；所有追问留在主页、固定聊天/未知命令，零猜测动作、模型或动作调用 |
| ROUTE-004 | DONE | BASE-006,ROUTE-002 | 聊天中的“帮我做”转成 `ActionRequest`，不直接执行 | 有类型的用户复杂任务只生成不可变 schema-v1 高风险 pending dry-run；路由交接仅预填可编辑 Phone Agent 文本，点击规划前必须先创建该请求，配置的规划渠道才能接收私有正文；仍须用户另行确认；39 条矩阵拒绝助手确认伪造、聊天/短命令、歧义和非法元数据，零模型/动作调用 |
...
| TOOL-003 | VERIFY | TOOL-002 | 协调器把目录上限内超时绑定到一次性权限，最多调用一个适配器；超时/取消中断 worker，范围/幂等重放拒绝，结果私有且有界。可见、默认取消的本地调用方现在只会消费既有 gateway result，执行 `Pictures/XiaoheiTest/` 项目数的只读查询；bridge 仍只解析受审的 MediaStore 测试/日历测试适配器。设备证据、可变适配器的权限/回滚、可信外部 peer 和资源关闭仍未验证 | 不用合成故障冒充设备证据 |
...
| TOOL-008 | VERIFY | TOOL-001 | 封闭的 `xiaohei-test` 日历适配器只预览有界元数据，在受审输入后创建无提醒事件，并只回滚当前内存实例创建的 event ID。任意账户/日历内容和批量操作均拒绝；用户运行时权限、网关接线和设备回滚仍必须补齐 | 测试账号，不读取个人日历 |
| TOOL-009 | VERIFY | TOOL-001 | Android 媒体边界目前只提供媒体音量相对 ±1 和输出路由观察（蓝牙/有线/扬声器或系统）；不能播放/暂停会话或强制切路由。活动会话、焦点、蓝牙/耳机/扬声器矩阵和设备证据仍必须补齐 | 来电/耳机/锁屏矩阵 |
| TOOL-010 | VERIFY | TOOL-003 | 用户启用的无障碍服务现可在受审包内精确选择标签，以及执行一次受限的返回/滚动；通用批准标签和自动文本填入一律拒绝。可见的逐请求文本确认、网关接线、App/版本矩阵和真机证据仍必须补齐 | 不点击通用“允许/确认/下一步” |
| TOOL-011 | VERIFY | TOOL-010 | 封闭 revision-1 注册表覆盖 15 个受审包名目标，提供精确选择/一次滚动/一次返回与类型化 fail-closed 失败说明；不声称已验证安装 UI 版本。网关调用方、逐 App/版本真机矩阵、页面变化处理和文本确认路径仍必须补齐 | 每 App 正常/拒绝/页面变化用例 |
| TOOL-012 | VERIFY | TOOL-003 | 结果门已接到每次受限无障碍点击/返回/滚动：适配器确认后，只有取得更新且满足预期前台包名的观察才会接受；过期/不匹配证据、适配器失败和重复验证均 fail-closed。仍需真实且已审核的设备/App/版本证据；这条窄链路不等于已证明可逆适配器目录 | 结果不能仅依赖退出码或模型描述 |
...
| OC-009 | VERIFY | OC-008 | 内存本地/Web 所有权状态仅向已验证的不透明 web-session 句柄转移控制权；重复/错误/终态转移均拒绝，任何转移都不能启动、恢复或复制任务；仍需真实 Web 交接证据 | 接管/归还任务所有权，不重复执行 |
...
| UX-003 | DONE | PLAN-001 | 只读任务卡投影受限本地 Phone Agent 生命周期：已批准目标摘要、审核/当前步骤、预算、已验证/失败结果和接管；不含任务正文/路径/Token/模型输出/推理，也不授予执行权限 | 不显示内部思维链或凭据 |
| UX-004 | DONE | POLICY-002 | 固定双语确认预览展示 App、目标、内容、权限和停止/回滚；取消仍为默认，且不授予或执行任何能力 | 不倒计时确认，也不预选同意 |
| UX-005 | VERIFY | TOOL-003 | 封闭的应用级停止中心向显式登记的所有者仅 fan-out 一次，暴露释放失败，并通过注销句柄避免滞留。主页/状态通知覆盖语音/DSP/CPU，Conversation 已登记在途聊天/TTS，活跃 Phone Agent 任务会登记并在终态/销毁时释放 owner，受权工具执行已登记取消信号；OpenCode、root 与设备资源归零仍需证据 | 1 秒内状态变为 stopping；最终资源归零 |
| UX-006 | VERIFY | TOOL-012 | Agent UI 的脱敏轨迹存储现默认关闭，并提供显式启用、删除最新一条、清空和关闭并清空。它不是工具结果历史：真实结果证据接线和独立设备行为仍需验证 | 清除后私有目录无残留正文/图片 |
