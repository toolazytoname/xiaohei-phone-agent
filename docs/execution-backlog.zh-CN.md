...
| UX-005 | VERIFY | TOOL-003 | 封闭的应用级停止中心向显式登记的所有者仅 fan-out 一次，暴露释放失败，并通过注销句柄避免 Activity 滞留。主页/状态通知覆盖语音/DSP/CPU，Conversation 已登记在途聊天/TTS；Phone Agent、工具、OpenCode、root 与设备资源归零仍需证据 | 1 秒内状态变为 stopping；最终资源归零 |
