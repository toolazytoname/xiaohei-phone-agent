...
| ROUTE-004 | DONE | BASE-006,ROUTE-002 | 聊天中的“帮我做”转成 `ActionRequest`，不直接执行 | 有类型的用户复杂任务只生成不可变 schema-v1 高风险 pending dry-run；路由交接仅预填可编辑 Phone Agent 文本，仍须用户另行请求规划和确认；39 条矩阵拒绝助手确认伪造、聊天/短命令、歧义和非法元数据，零模型/动作调用 |
