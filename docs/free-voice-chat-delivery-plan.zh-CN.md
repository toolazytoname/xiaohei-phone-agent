...
- [ ] 模型回复即使包含 JSON、工具名或“已替你完成”，也只能显示/播报。
- [ ] 只有用户原始语音可进入确定性短命令路由；模型回复永不进入。
- [ ] 复杂任务只生成 Phone Agent 可编辑草稿，必须审阅计划/目标/风险/回滚并新鲜确认。
- [ ] 语音“确认”不能代替 L2/L3 动作的可见确认，除非未来另过防回放、身份和内容绑定门禁。
- [ ] 支付、转账、OTP、密码和规避风控永久拒绝。

证据：20 条聊天注入/伪工具零动作；短命令精确一次；复杂任务只到草稿；取消后零计划/动作。

### FVC-110 — 自动化、AOSP 与 OnePlus 验收

依赖：目标工作包代码完成。

- [ ] 纯 Java：状态机、profile 隔离、代际取消、重复 final、预算、控制短语、隐私、音频 lease、失败指纹。
- [ ] 静态门禁：Conversation 无动作路径；release 无正文日志；Conversation Token 只进其 Authorization header。
