...
Static enforcement rejects any classifier reference to Android APIs, action dispatch, model clients, tool gateways, shell, or network. It also confirms `MainActivity` does not use the classifier yet. User-path integration waits for `ROUTE-003` low-confidence clarification and `ROUTE-004` unconfirmed `ActionRequest`, preventing this foundation from silently changing current command behavior.
