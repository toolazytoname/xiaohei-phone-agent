...
| VOICE-005 | VERIFY | VOICE-004 | Signal-only policy maps call, alarm, media and Activity interruption to stop input/output and release ownership with no auto-resume. OnePlus Activity/background evidence proves a real local-ASR capture is released with no active record client and DSP remains armed; genuine call/alarm/media signals still require evidence; see `acceptance-voice-005.md` |
...
| VOICE-011 | HUMAN | VOICE-010 | Conversation UI has a separate visible stop-speech control; accepted/repeated replies use the selected system TTS only, and stop/clear/lifecycle paths interrupt it without auto-resume. OnePlus engine-visible output stopped 227 ms after the control event and the queued remainder was cancelled; human audible ≤300ms interruption and echo-loop qualification remain required; see `acceptance-voice-011.md` |
...
| CHAT-005 | VERIFY | CHAT-004,VOICE-002 | Separate open-conversation ASR, single/multi-turn push-to-talk, and DSP/interruption code gates are complete; see the [free-voice execution plan](free-voice-chat-delivery-plan.md) | AOSP/OnePlus/HUMAN evidence must still show spoken question → text → model → offline TTS, global stop, no input/output overlap, and no automatic microphone reopening |
...
| ROUTE-002 | DONE | CHAT-004 | Three-way classifier has an exact 100-case matrix: 40 deterministic commands retain the existing action path, 35 chats only prefill Conversation, and 25 complex tasks only prefill visible Phone Agent; all handoffs start zero model/action calls |
| ROUTE-003 | DONE | ROUTE-002 | Pure local clarification passes an exact 50-case matrix: 10 target, 10 intent, 10 scope asks and 20 clear controls; every ask stays on Home with chat/unknown and zero guessed actions, model calls, or action calls |
| ROUTE-004 | DONE | BASE-006,ROUTE-002 | Typed user complex-task requests remain immutable schema-v1 high-risk pending dry runs; route handoff only prepopulates editable Phone Agent text, and a Planning click must create that request before the configured planner may receive private text. It still requires separate user confirmation; 39-case matrix rejects assistant confirmation forgeries, chats/commands, ambiguities, and invalid metadata with zero model/action calls |
...
| TOOL-003 | VERIFY | TOOL-002 | Coordinator binds catalog-capped timeout into one-use authority, limits execution to one adapter, interrupts timeout/cancel workers, rejects scope/idempotency replay, and returns bounded private structured errors. A visible cancel-first local caller now consumes the existing gateway result for only a read-only `Pictures/XiaoheiTest/` count query; the bridge resolves only reviewed MediaStore-test/calendar-test adapters. Device evidence, mutating-adapter permission/rollback, trusted external peer and teardown evidence remain unverified |
...
| UX-003 | DONE | PLAN-001 | Read-only task card projects bounded local Phone Agent lifecycle: approved target summary, reviewed/current steps, budget, verified/failure result and takeover. It exposes no task prose/paths/tokens/model output/reasoning and grants no execution authority |
...
| UX-005 | VERIFY | TOOL-003 | Closed application-level stop hub fans out once to explicitly registered owners, exposes failed release, and provides unregister handles to avoid retention. Home/status stop covers voice/DSP/CPU, Conversation registers pending chat/TTS, an active Phone Agent task registers then releases its owner on terminal/destroy, and authorized tool execution registers its cancellation signal; OpenCode, root and device proof of zero resources remain required |
