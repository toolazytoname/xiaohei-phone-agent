...
| OpenCode 复杂任务 / OpenCode complex tasks | `FOUNDATION`; `OC-002/003` done | 手机上的 OpenCode TUI/Web、独立模型 profile、三类型私有任务协议，以及 task-private input/output 路径隔离 | `OC-004`; bounded runner before any execution |
...
1. `OC-004` → `OC-008` — 接入受控 OpenCode 能力，不把原始 shell 暴露给模型。
