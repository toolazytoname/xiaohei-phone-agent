...
This foundation is intentionally not wired to `MainActivity` or `ConversationActivity`. `PLAN-001` must define the bounded planner output, and `POLICY-002` must define a fresh confirmation transition bound to task, target, content, expiry, and device state. Until those gates exist, the request stops at `pending`.
