...
规则优先规划器可以使用目录风险元数据拒绝未知工具或风险不匹配，但它生成的仍只是 dry-run 提案。本任务没有新增 Activity/Service 接线，也没有 capability token。必须由 `TOOL-002` 独立建立仅 loopback 的网关，以及一次性、短时、绑定任务的授权，目录项才能到达适配器。
