...
| TOOL-003 | VERIFY | TOOL-002 | 协调器把目录上限内超时绑定到一次性权限，最多调用一个适配器；超时/取消中断 worker，范围/幂等重放拒绝，结果私有且有界。封闭 Android 注册表现在只解析受审的 MediaStore 测试和日历测试适配器；真实权限、网关调用方、资源关闭和设备证据仍未验证 | 不用合成故障冒充设备证据 |
...
| UX-005 | VERIFY | TOOL-003 | 封闭八类别内存全局停止注册表向显式登记的资源所有者仅 fan-out 一次，并暴露任何释放失败；现有主页按钮及其状态通知 Intent 已用于 MainActivity 的语音/DSP/CPU 所有者，其他 App/通知/语音/微件集成及设备上的资源归零仍需证据 | 1 秒内状态变为 stopping；最终资源归零 |
