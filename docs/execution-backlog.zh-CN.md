...
| ID | 状态 | 依赖 | 交付物 | 完成证据 |
|---|---|---|---|---|
| PROGRESS-001 | READY | BASE-004 | 每个 PR 更新 `STATUS.md` 的 Now/Next/Blocked/Evidence | PR 模板或校验检查缺失状态更新 |
| PROGRESS-002 | BACKLOG | PROGRESS-001 | GitHub Issue 模板包含任务 ID、依赖、验收、回滚和人类门禁 | 新 Issue 可直接交给较弱模型 |
| PROGRESS-003 | BACKLOG | PROGRESS-002 | GitHub Project 五列和标签映射 | 人类能从手机查看 Now/Verify/Human gates |
| PROGRESS-004 | BACKLOG | PROGRESS-001 | 生成只读状态摘要的脚本，不自动修改证据 | 输出任务状态、最近 PR、阻断和下一项 |
