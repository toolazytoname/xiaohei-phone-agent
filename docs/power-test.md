...
3. Turn the display off with the power button, then physically unplug USB/power within ten minutes. This matters on devices configured to stay awake while charging: sampling begins only after Android reports AC, USB, and wireless power all false, no active call, and non-interactive wakefulness. The raw TSV records both the requested and actual sampling-start epochs plus the preflight wait, so elapsed time is never inferred from the host launch time.
...
The monitor samples battery level/status/current where exposed, temperature, thermal status, active recording, Xiaohei-named wakelocks, wakefulness, current call state, and whether power was reattached. It stores no phone number, foreground app, notification, or user content. It does not keep USB alive and needs no Mac connection after launch. Acceptance requires `# COMPLETE`, the full elapsed duration, and zero recorder, Xiaohei wakelock, awake, active-call, or powered samples. Any such sample invalidates the run instead of being silently averaged away. A device-hidden current sensor is reported honestly.
