...
| 可见 Phone Agent / Visible Phone Agent | `DONE` in M5 scope; `TOOL-002` auth core done | AOSP 十 App 10/10、全局停止、包绑定、内存视觉恢复；路由、计划、确认、工具目录与 loopback/same-UID capability 授权边界通过 | `TOOL-003` execution lifecycle |
| 开放对话 / Open conversation | `VERIFY`; automated `CHAT-012` passed | 前述对话基础、确定性离线 FAQ、20/5/5/5 自动矩阵、五类请求前隐私拒绝及 AOSP 零录音残留已验证 | Human Mandarin playback/intelligibility/interruption remains; agent path proceeds with `TOOL-003` |
...
1. `TOOL-003` — 在已完成但未接线的授权核心后补每工具超时、取消、幂等与结构化错误；完成前仍不接 Android 适配器。
...
- `TOOL-002`：一次成功确认的私有 receipt 只能换取一个纯内存、一次性、默认 `SecureRandom` 128-bit、1–30 秒 capability；它绑定 confirmation/task/request/plan/call/工具/版本/风险/受众/参数/幂等/时间/隐私摘要。50 组 Java 与 7 个公开 fixture 文件拒绝 10 个非 loopback/跨 UID、确认复用/错范围、7 种调用漂移、3 种目录漂移、5 种非法/未来/陈旧/私有元数据调用、精确到期/时钟回退、重放/撤销及跨网关；活动/重放状态有界，模型/动作/执行调用均为 0。当前没有监听器、UI 或适配器执行，下一门禁为 `TOOL-003`。
...
- `PLAN-001`：版本化 `task-plan.v1` 强制绑定 request、dry-run、1–8 步与 1–60 秒边界；34 条 Java 矩阵和 5 个公开夹具覆盖前向/线性 DAG、8 步边界、5 个未知工具、9 步拒绝、风险/版本/幂等/依赖/参数失败及 5 个真实循环，全部零模型/动作调用。确认、目录和授权核心已完成；校验器仍未接 UI、模型或执行器，下一门禁是 `TOOL-003`。
