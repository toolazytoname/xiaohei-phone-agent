...
The short command session now requests Android transient exclusive speech focus before listening. If the system revokes focus (including an exclusive audio interruption such as a call), it cancels recognition, releases the microphone, and exposes an explainable state. The device debug build also emits redacted lifecycle markers (`session_started`, `audio_focus_lost`, and `session_stopped microphone_released=true`); they contain neither transcript nor call identity. A call received outside an active listening session can prove only that no recording client remains afterwards, not the interruption path. Open M2 gate: one real incoming-call interruption run while Xiaohei is listening; the equivalent activity interruption path is already verified, but code review does not replace it. Accessibility, root shell, automatic dialing, and background submission are not used.
