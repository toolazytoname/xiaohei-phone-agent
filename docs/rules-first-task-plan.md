...
The validator checks the current [seven-entry reviewed versioned catalog](versioned-tool-catalog.md): Open Settings, Open Gallery, Open Dialer, Adjust Volume, Observe, the closed MediaStore test collection, and the closed calendar test account. An unknown name such as `root.shell`, `android.tap`, or `opencode.run` is denied, as is a known tool with a mismatched risk. This does not pre-approve the tools for execution; it only proves that a proposed plan uses known names and declared risks.
