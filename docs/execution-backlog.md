...
| TOOL-003 | VERIFY | TOOL-002 | Pure coordinator binds catalog-capped timeout into one-use authority, limits execution to one injected adapter, interrupts timeout/cancel workers, rejects scope/idempotency replay, and returns bounded private structured errors; 25 Java groups and five result fixtures pass, while real process/network adapter teardown remains unverified |
