...
| `ROUTE-004` — 未确认 ActionRequest | 已完成任务范围 | 仅接受有类型用户消息的 schema-v1 工厂；不可变高风险 pending dry-run；39 条合成矩阵含 10 条助手确认伪造；4 个公开 Schema 夹具；零模型/动作调用；矛盾 pending/live 与 pending/免确认载荷拒绝。 | 有意尚未接入 `MainActivity`/`ConversationActivity`；可见动作升级或确认转换前必须先完成 `PLAN-001` 与 `POLICY-002`。 |
