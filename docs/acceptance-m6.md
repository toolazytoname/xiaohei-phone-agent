...
The candidate is not a public Beta yet. Required evidence still missing: 8–24 hour unplugged idle/power, an independent malware-engine review beyond static/OSV checks, production signing-key governance, model redistribution approval, a real user-config backup/restore drill, and a genuinely production-signed reproducible APK. The 25-surface launch matrix does not replace the separate 10–15-app Phone Agent task matrix.
