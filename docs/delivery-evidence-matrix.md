...
| M6 — public Beta hardening | partial | Debug/release separation, exact APK scan/SBOM/diagnostics, upgrade/downgrade/uninstall, 100-action stress, 25 surface launches, and a non-secret model-channel backup format. | 8–24 hour physical unplugged idle/power, independent malware-engine review, production key governance, model redistribution approval, a real user-config backup/restore drill, reproducible production-signed APK. |
