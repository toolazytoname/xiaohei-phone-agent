...
| M5 — visible Phone Agent | partial | Preview-before-execute, local policy, package binding before/after actions, global stop, Settings two-step task, Calculator task, bounded redacted trace, and consent-gated local visual recovery that never uploads or persists screenshots. | 10–15 common-app task matrix and real multi-step recovery. |
