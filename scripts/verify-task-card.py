...
for text, terms in ((core, ("UNAVAILABLE", "TAKEN_OVER", "不显示任务正文、路径、Token、模型回复或推理过程")), (test, ("takeover=1", "execution=0")), (ui, ("TaskCardProjection.unavailable", "只读任务卡", "beginTaskCard", "refreshTaskCard", "lastTaskResult"))):
 if any(term not in text for term in terms): raise SystemExit("FAIL task-card boundary")
print("PASS task-card target/plan/budget/result/takeover public=true bounded_lifecycle=1 task_text=0")
