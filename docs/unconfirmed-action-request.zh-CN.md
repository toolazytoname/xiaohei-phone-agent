...
本基础目前有意不接入 `MainActivity` 或 `ConversationActivity`。`PLAN-001` 已定义有边界的 dry-run 计划输出；`POLICY-002` 仍须定义绑定任务、目标、内容、有效期与设备状态的新鲜确认转换，`PLAN-002` 再提供最小远端适配。在这些门禁完成前，请求只能停在 `pending`。
