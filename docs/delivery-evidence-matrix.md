...
| M6 — public Beta hardening | partial | Debug/release separation, external release identity, exact APK scan/SBOM/diagnostics, upgrade/downgrade/uninstall, 100-action stress, 25 surface launches, non-secret model-channel backup, byte-identical debug and internal release builds. | 8–24 hour physical unplugged idle/power, independent malware-engine review, encrypted offline key recovery, model redistribution approval, a real user-config backup/restore drill, public-release provenance. |
